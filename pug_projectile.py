...
background, pug = [], []
pug_traits = {"facing": "R", "head": head_side}
...
def update_pug(traits):
    ...
    pug = []
    for x in range(4):
        pug.append(tail[x] + (pug_traits["head"])[x])
...
def game(win):
    ...
    while True:
        ...
        distance = 100
        loop_delay = .1
        window.addstr(9, 5, "PUG PROJECTILE  -  by hzlnutButter", curses.A_BOLD)
        menu_lines = [("Eat as many bones as you can on your way to Mars, " + str(distance) + " million kilometres away."), "If you are less than 30 kg, the planet's gravitational pull will be too weak.", "Big bones 8===8 are 1 kg each, and small bones 8=8 are 0.5 kg.", "", "COMMANDS: move left -> A", "          move right -> D", "          pause -> SPACE", "          restart -> RETURN", "", "PRESS RETURN TO BEGIN"]
        for i in range(len(menu_lines)):
            window.addstr(11+i, 5, menu_lines[i])
        ...
        while distance > 0:
            ...
            try:
                ...
                if key == " ": # space (pause)
                    ...
                    break
                elif key == "a" or key == "A": # a or A
                    pug_location += -5
                    pug_traits["facing"] = "L"
                    if pug_location < 1:
                        pug_location = 1
                elif key == "d" or key == "D": # d or D
                    pug_location += 5
                    pug_traits["facing"] = "R"
        ...
        if distance > 0: # you didn't make it
            ...
            try:
                if pug_traits["facing"] == "R":
                    ...
                    for y in range(3):
                        window.addstr(y+11, (pug_location+5+y), pug[y+5])
                    for y in range(3):
                        window.addstr(y+14, (pug_location+8), pug[y+8])
            except Exception:
                break
            window.border(0)
            window.refresh()
            time.sleep(1)
            display_text = "YOU HAD " + str(int(distance)) + " KM LEFT TO GO."
            ...
            for i in range(14):
                ...
                while time.time() - start_time < .15:
                    continue
            display_text = "YOU NEEDED " + str(30-mass) + " KG MORE OF TREATS."
        else: # win
            update_pug({"head": head_front})
            yval = height-3
            for i in range(int((height-15) / 2)):
                ...
                while time.time() - start_time < .15:
                    continue
            display_text = "CONGRATULATIONS, YOU MADE IT! NOW YOU CAN MAKE A NEW HOME ON MARS."
        window.clear()
        if mass > highest_mass:
            window.addstr(6, 5, ("NEW HIGH SCORE: " + str(mass) + " kg"), curses.A_BOLD)
            highest_mass = mass
